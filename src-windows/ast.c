 ...
 * 
 * It is available for the Windows operating systems.
 *
 */
#include "ast.h"
#include <stdlib.h>
...
void ast_node_destroy(ASTNode *node) {
    ...
    if (node->params) {
        for (size_t i = 0; i < node->param_count; i++) {
            ...
            // default_value is an ASTNode subtree not stored as a child —
            // destroy it here to avoid leaks on both parse and XBC paths.
            ast_node_destroy(node->params[i].default_value);
        }
        free(node->params);
    }
    // Free type params
