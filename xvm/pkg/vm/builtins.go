...
func RegisterBuiltins(env *Env) {
	...
	sysObj := Object(map[string]*Value{
		...
		"readFile": builtin(func(a []*Value) (*Value, error) {
			if len(a) == 0 {
				return Null(), nil
			}
		...
		"writeFile": builtin(func(a []*Value) (*Value, error) {
			if len(a) < 2 {
				return Null(), nil
			}
			err := os.WriteFile(a[0].String(), []byte(a[1].String()), 0644)
