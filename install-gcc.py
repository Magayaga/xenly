...
def compile_with_gcc():
    ...
    compile_command = ["gcc"] + source_files + ["-o", output_file, "-lm"]
    ...
    if compile_status == 0:
        ...
        print("UPDATE LEVEL!")  # Indicate successful update or version
    else:
...
def compile_math_library():
    ...
    if platform.system() == "Windows":
        ...
        compile_command = f"gcc {source_file} -shared -o {output_file} -lm"
    elif platform.system() == "Linux":
        output_file = "math.so"
        compile_command = f"gcc {source_file} -shared -o {output_file} -fPIC -lm"
    else:
